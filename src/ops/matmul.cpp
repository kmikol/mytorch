...
Tensor MatMulOp::forward(const Tensor& A, const Tensor& B) {
    ...
    // Strides let the same kernel handle both contiguous and transposed tensors.
    // .T() swaps strides without copying data, so A[m,k] = a[m*sA0 + k*sA1]
    // works for both layouts.
    size_t sA0 = A.strides[0], sA1 = A.strides[1];
    size_t sB0 = B.strides[0], sB1 = B.strides[1];

    #ifdef __ARM_NEON
    // NEON micro-kernel path — used when B is contiguous in the column
    // direction (sB1==1), i.e. normal (non-transposed) B.
    // The backward pass always passes a transposed B so it falls through.
    if (sB1 == 1) {
        neon_matmul_forward(c, a, sA0, sA1, b, sB0, M, K, N);
        return C;
    }
    #endif

    // Scalar fallback — handles non-contiguous B (e.g. transposed inputs).
    // Three-level cache tiling keeps the working set in L1.
    // Inner loop order m-k-n hoists a[m,k] outside the n loop.
